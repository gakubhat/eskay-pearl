...
export default class ReportType extends Component {
    ...
    componentDidMount() {
        $('.ui.dropdown.reportType').dropdown({
    ...
    componentDidUpdate() {
        ...
        $('.ui.dropdown.reportType').dropdown("restore defaults",0);
        $('.ui.dropdown.reportType').dropdown("set selected",0);
        store.selectedReport= $('.ui.dropdown.reportType').dropdown("get text")
    }


    reportChanged = (value, text, $selectedItem)=> {
        console.log("Report Changed ",text)
        store.selectedReport = text || "";
    };

    render() {

        let reportTypeMenu = store.reportsToShow.map((report, index)=>
            <div className="item" data-value={index} data-text={report.name} key={index}>{report.name}</div>
        )
        return <div>
            <div className="ui  segment">
                <span className="ui  text">ಮಾಹಿತಿ: </span>
